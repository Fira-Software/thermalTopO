#!/usr/bin/env python3
"""FD verification of the patch p-norm temperature objective path."""
...
# base full run
use("pnorm"); clean(); run_solver("log.pn_base")
J0 = get_J("log.pn_base")
sens = readf("1/topologySensas1")
shutil.copy("1/topologySensas1", "fd_topologySens.pnorm")
...
use("pnorm_primalonly")
rows = []
for name, d in dirs:
    Js = {}
    for pm, sg in (("+",1.0),("-",-1.0)):
        write_alpha([alpha0[i] + sg*EPS*d[i] for i in range(len(sens))])
        clean(); run_solver(f"log.pn_{name}{pm}")
        Js[pm] = get_J(f"log.pn_{name}{pm}")
    fd = (Js["+"]-Js["-"])/(2*EPS)
...
for k,(i,grp) in enumerate(picks):
    ...
    for pm, sg in (("+",1.0),("-",-1.0)):
        ...
        write_alpha(vals); clean(); run_solver(f"log.pnc_{i}{pm}")
        Js[pm] = get_J(f"log.pnc_{i}{pm}")
    fd = (Js["+"]-Js["-"])/(2*EPS)
    cells.append((i,grp,sens[i],fd))
    print(f"[{k+1}/{len(picks)}] cell {i} ({grp}) sens={sens[i]:.6g} FD={fd:.6g}", flush=True)

write_alpha(alpha0); use("full")
