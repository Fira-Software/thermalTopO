...
# base full run
use("pnorm_regT"); clean(); run_solver("log.pnt_base")
J0 = get_J("log.pnt_base")
sens = readf("1/topOSensas1")
shutil.copy("1/topOSensas1", "fd_topologySens.pnorm_regT")
...
use("pnorm_regT_primalonly")
rows = []
for name, d in dirs:
    Js = {}
    for pm, sg in (("+",1.0),("-",-1.0)):
        write_alpha([alpha0[i] + sg*EPS*d[i] for i in range(len(sens))])
        clean(); run_solver(f"log.pnt_{name}{pm}")
        Js[pm] = get_J(f"log.pnt_{name}{pm}")
    fd = (Js["+"]-Js["-"])/(2*EPS)
...
for k,(i,grp) in enumerate(picks):
    ...
    for pm, sg in (("+",1.0),("-",-1.0)):
        ...
        write_alpha(vals); clean(); run_solver(f"log.pntc_{i}{pm}")
        Js[pm] = get_J(f"log.pntc_{i}{pm}")
    fd = (Js["+"]-Js["-"])/(2*EPS)
